...
showHeader($Project->data['titre'],false,false,true);?>
	...
	<?php $cpt=0;foreach($Project->getDocs() as $id_doc => $doc) {$cpt++;
		...
		$mode = mode_image($doc);
	?>
		<a style="height:<?php echo $GLOBALS['MIN_CELL_HEIGHT'];?>px;width:<?php echo round($GLOBALS['MIN_CELL_HEIGHT']*$mode['ratio']);?>px;" data-mode="<?php echo $mode['mode'];?>" data-ratio="<?php echo $mode['ratio'];?>" href="#i<?php echo $cpt;?>" class="toup cell project thumb">
