...
class MySQL {
	...
	function Create($table,$champs,$drop=false) {
		if($drop) {
			$this->ExecuteSQL('DROP TABLE `'.addslashes($table['name']).'`');
		}
		$primary_key = '';
		$SQL=array('CREATE TABLE IF NOT EXISTS `'.addslashes($table['name']).'` (');
		foreach($champs as $k=>$v) {
			...
			$SQL[]='`'.
				...
				$ai.' '.
				($v['comment'] ? ' COMMENT \''.addslashes($v['comment']).'\'': '').' ';
		}
		if($primary_key) {
			$SQL[]=', PRIMARY KEY (`'.$primary_key.'`)';
		}
		$SQL[]=') ENGINE=MyISAM DEFAULT CHARSET=utf8 AUTO_INCREMENT=1 COMMENT=\''.addslashes($table['comment']).'\';';

		return $this->ExecuteSQL(implode("\n",$SQL));
	}
	...
	function ArrayResult(){
		...
		list($k,$v)=each($this->arrayedResult);
		if(!is_numeric($k)) {
